  ...
  const handleProceedToQRIS = async () => {
    ...
    try {
      ...
      if (response.ok && resData.qrUrl) {
        ...
        alert("Gagal membuat QRIS: " + (resData.error || "Cek konfigurasi Midtrans"));
      }
    } catch (error) {
      console.error(error);
          ...
          <div className="relative w-full h-[400px] rounded-3xl overflow-hidden bg-[#123524] flex items-center p-12 shadow-2xl border border-white/5">
            <div className="relative z-10 max-w-md">
              ...
              <p className="text-gray-400 text-sm mb-8 font-medium leading-relaxed">
                Nikmati kesegaran buah asli pilihan langsung dari petani lokal untuk harimu yang lebih sehat.
              </p>
              <a href="#products" className="bg-[#c2aa6b] text-[#061e12] font-black py-4 px-10 rounded-full shadow-xl hover:shadow-[#c2aa6b]/20 transition transform hover:-translate-y-1 inline-block">
                PESAN SEKARANG
              </a>
            </div>
          </div>
              ...
              {modalView === "detail" && (
                <>
                          ...
                          <div key={id} className="flex justify-between items-center text-sm font-medium bg-gray-50 p-3 rounded-xl border border-gray-100">
                            ...
                            <div className="w-1/2 flex flex-col items-end">
                              <span className="text-gray-400 text-xs font-bold">
                                {qty}x {Number(product.harga).toLocaleString("id-ID")}
                              </span>
                              <span className="font-black text-[#123524]">
                                Rp {(Number(product.harga) * qty).toLocaleString("id-ID")}
                              </span>
                            </div>
                          </div>
                        );
                  ...
                  <div className="flex justify-end">
                    <button onClick={handleProceedToQRIS} className="bg-[#123524] text-white px-10 py-4 rounded-full font-black shadow-lg shadow-[#123524]/30 hover:bg-black transition w-full md:w-auto">
                      PROSES PEMBAYARAN ➔
                    </button>
                  </div>
                </>
...
function ProductCard({ product, qty, onAdd, onRemove, isBest }: any) {
  return (
    <div className={`bg-white rounded-2xl p-3 flex flex-col items-center shadow-lg transition transform hover:-translate-y-2 relative group ${isBest ? "ring-4 ring-[#c2aa6b]/30 shadow-[#c2aa6b]/20" : ""}`}>
      {isBest && (
        <div className="absolute -top-3 -right-3 bg-[#b54a4a] text-white text-[10px] font-black px-3 py-1.5 rounded-lg shadow-xl border-2 border-white z-10 animate-bounce">🔥 TERLARIS</div>
      )}
